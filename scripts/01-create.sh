...
# MariaDB / Recursor / dnsdist use IMAGE + IMAGE_TAG.
# PowerDNS Authoritative / PowerAdmin default to version-derived images.
...
_resolve_auth_image() {
  local derived_image
  derived_image="$(derive_auth_image_from_version "${PDNSSTACK_AUTH_VERSION}")"

  PDNSSTACK_AUTH_IMAGE="${PDNSSTACK_AUTH_IMAGE:-${PDNSSTACK_POWERDNS_AUTH_IMAGE:-${derived_image}}}"
  PDNSSTACK_AUTH_IMAGE_FULL="${PDNSSTACK_AUTH_IMAGE_FULL:-${PDNSSTACK_POWERDNS_AUTH_FULL_IMAGE:-}}"

  if [[ -z "${PDNSSTACK_AUTH_IMAGE_FULL}" ]]; then
    if [[ -n "${PDNSSTACK_POWERDNS_AUTH_IMAGE_TAG:-}" ]]; then
      PDNSSTACK_AUTH_IMAGE_FULL="${PDNSSTACK_AUTH_IMAGE}:${PDNSSTACK_POWERDNS_AUTH_IMAGE_TAG}"
    else
      PDNSSTACK_AUTH_IMAGE_FULL="${PDNSSTACK_AUTH_IMAGE}"
    fi
  fi

  PDNSSTACK_POWERDNS_AUTH_FULL_IMAGE="${PDNSSTACK_AUTH_IMAGE_FULL}"

  export PDNSSTACK_AUTH_VERSION
  export PDNSSTACK_AUTH_IMAGE
  export PDNSSTACK_AUTH_IMAGE_FULL
  export PDNSSTACK_POWERDNS_AUTH_FULL_IMAGE
}

_resolve_poweradmin_image() {
  PDNSSTACK_POWERADMIN_IMAGE="${PDNSSTACK_POWERADMIN_IMAGE:-docker.io/poweradmin/poweradmin}"
  PDNSSTACK_POWERADMIN_IMAGE_FULL="${PDNSSTACK_POWERADMIN_IMAGE_FULL:-${PDNSSTACK_POWERADMIN_FULL_IMAGE:-}}"

  if [[ -z "${PDNSSTACK_POWERADMIN_IMAGE_FULL}" ]]; then
    local tag="${PDNSSTACK_POWERADMIN_IMAGE_TAG:-${PDNSSTACK_POWERADMIN_VERSION}}"
    PDNSSTACK_POWERADMIN_IMAGE_FULL="${PDNSSTACK_POWERADMIN_IMAGE}:${tag}"
  fi

  PDNSSTACK_POWERADMIN_FULL_IMAGE="${PDNSSTACK_POWERADMIN_IMAGE_FULL}"

  export PDNSSTACK_POWERADMIN_VERSION
  export PDNSSTACK_POWERADMIN_IMAGE
  export PDNSSTACK_POWERADMIN_IMAGE_FULL
  export PDNSSTACK_POWERADMIN_FULL_IMAGE
}
...
mkdir -p "${CONFIG_DIR}"
RUNTIME_BACKUP_SCHEDULE="$(printf '%q' "${PDNSSTACK_BACKUP_SCHEDULE}")"
...
PDNSSTACK_POWERADMIN_HTTP_PORT=${PDNSSTACK_POWERADMIN_HTTP_PORT}
PDNSSTACK_BACKUP_SCHEDULE=${RUNTIME_BACKUP_SCHEDULE}
PDNSSTACK_AUTH_VERSION=${PDNSSTACK_AUTH_VERSION}
PDNSSTACK_POWERADMIN_VERSION=${PDNSSTACK_POWERADMIN_VERSION}
PDNSSTACK_AUTH_IMAGE=${PDNSSTACK_AUTH_IMAGE}
PDNSSTACK_AUTH_IMAGE_FULL=${PDNSSTACK_AUTH_IMAGE_FULL}
PDNSSTACK_POWERADMIN_IMAGE=${PDNSSTACK_POWERADMIN_IMAGE}
PDNSSTACK_POWERADMIN_IMAGE_FULL=${PDNSSTACK_POWERADMIN_IMAGE_FULL}
PDNSSTACK_MARIADB_FULL_IMAGE=${PDNSSTACK_MARIADB_FULL_IMAGE}
PDNSSTACK_POWERDNS_RECURSOR_FULL_IMAGE=${PDNSSTACK_POWERDNS_RECURSOR_FULL_IMAGE}
PDNSSTACK_DNSDIST_FULL_IMAGE=${PDNSSTACK_DNSDIST_FULL_IMAGE}
EOF
